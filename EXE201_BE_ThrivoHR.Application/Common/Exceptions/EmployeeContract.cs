...
{
    public class NotFoundException(string name) : Domain.Common.Exceptions.NotFoundException($"Employee Contract ID {name} was not found.");
    ...
    public class ContractAlreadyEndedException(string name) : BadRequestException($"Failed to end contarct {name} because the contract has ended.");
